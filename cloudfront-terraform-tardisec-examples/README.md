# AWS CloudFront (Terraform) × tardisec

Drop-in files for applying the synced tardisec config at CloudFront with Terraform, in front of
S3, a Lambda function URL, an ALB, or any custom origin. No application code.

| File | What it does |
| --- | --- |
| `main.tf` | Builds the response headers policy from `.tardisec.json`, filtering what CloudFront will not take |
| `.tardisec.cloudfront-terraform.tf` | Synced. The same resource as the API emits it, every header as a custom header |
| `.tardisec.json` | Synced. DNS records, the full header map, and `/.well-known/` contents |

Reading the manifest at plan time is what keeps the policy and the sync in step: no generated
HCL to review, and `terraform plan` is the diff. Attach the policy to every behavior that
serves HTML. On CloudFormation instead?
[cloudfront-cloudformation-tardisec-examples](../cloudfront-cloudformation-tardisec-examples).
...
`main.tf` if you do, since both declare
`aws_cloudfront_response_headers_policy.tardisec`. Either one is deployable: both apply the
split described below.

## What CloudFront will not take

The header map is bigger than a response headers policy can hold, so `main.tf` and the synced
file both split it:
...
- The six headers CloudFront gives a dedicated field (`Content-Security-Policy`,
  ...
  `X-Frame-Options`, `X-XSS-Protection`) are set in `security_headers_config` instead, with
...
- **Raise Service Quotas `L-8FE99263` before your first apply.** The policy sets 18 custom
  ...
  `security_headers_config` entries do not count against that quota.
